...
[CustomEditor(typeof(ARStillCamera))] 
public class ARStillCameraEditor : Editor 
{
    ...
    {
		ARStillCamera arsc = (ARStillCamera)target;
		if (arsc == null) return;
		...
		arsc.Stereo = EditorGUILayout.Toggle("Part of a stereo pair", arsc.Stereo);
		if (arsc.Stereo) {
			arsc.StereoEye = (ARStillCamera.ViewEye)EditorGUILayout.EnumPopup("Stereo eye:", arsc.StereoEye);
		}
		...
		arsc.Optical = EditorGUILayout.Toggle("Optical see-through mode.", arsc.Optical);
		...
		if (arsc.Optical) {
			...
			if (OpticalParamsFilenames.Length > 0) {
				int opticalParamsFilenameIndex = EditorGUILayout.Popup("Optical parameters file", arsc.OpticalParamsFilenameIndex, OpticalParamsFilenames);
				string opticalParamsFilename = OpticalParamsAssets[opticalParamsFilenameIndex].name;
				if (opticalParamsFilename != arsc.OpticalParamsFilename) {
					arsc.OpticalParamsFilenameIndex = opticalParamsFilenameIndex;
					arsc.OpticalParamsFilename = opticalParamsFilename;
					arsc.OpticalParamsFileContents = OpticalParamsAssets[arsc.OpticalParamsFilenameIndex].bytes;
				}
				arsc.OpticalEyeLateralOffsetRight = EditorGUILayout.FloatField("Lateral offset right:", arsc.OpticalEyeLateralOffsetRight);
				...
				arsc.OpticalParamsFilenameIndex = 0;
				EditorGUILayout.LabelField("Optical parameters file", "No parameters files available");
				arsc.OpticalParamsFilename = "";
				arsc.OpticalParamsFileContents = new byte[0];
			}
		}
    }
}
